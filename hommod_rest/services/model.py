...
def withoutHomomerDuplicates(chain_order, chain_sequences):
    """
    Many templates contain duplicate chains. This function reduces the set of
    sequences to a smaller set with only unique sequences.
    """
    ...
    if sameNum:  # If all sequences occur twice for example:
        ...
        for seq in count.keys():
            newchain_sequences[seq2chains[seq][0]] = seq

        for chain in chain_order:
            if chain in newchain_sequences:
                newchain_order.append(chain)

        return [newchain_order, newchain_sequences]
    else:
        return [chain_order, chain_sequences]
...
# I includes all the chain ids in 'chain_order' in the alignment.
def writeAlignmentFasta(chain_order, alignmentsByChain, template_ac, path):
    f = open(path, 'w')
...
class Modeler(object):
    ...
    def _build_for_domain(self, model_dir, main_target_id, uniprot_species_name,
        ...
        main_target_sequence = \
            adjusttarget_sequence(main_target_sequence,
...
# given target_seq. It returns that one's dictionary key.
def selectMostIdentical(target_seq, seqs):
    best = None
    ...
    for key in seqs.keys():
        ...
        d = {key: seqs[key], 'target': target_seq}
...
def selectLongestAligned(hits, minLen=0):
    """
    This function iterates through a dictionary of sequences and selects the
    one with the longest alignment to the given target_seq. It returns that
    sequence's dictionary key.
    """
...
def getIsoforms(seq, uniprot_species_name):
    """
...
def adjusttarget_sequence(target_seq, templateseq, uniprot_species_name):
    """
    ...
    isoformkeys = []
    isoformHits = getIsoforms(target_seq, uniprot_species_name)
