...
# returns true if the two list have at least one matching element.
...
# * the yasara object
# * the yasara instance that loaded the object
...
# This function assumes a conventional ID format and extracts the pdb accession code and chain id from it.
...
# Returns number of aligned residues
...
# Returns just the percentage idenity of an alignment
...
# Gets the seqres sequences from the rcsb website. Don't use this in high frequencies!
def getRCSBSeqs (ac):
    try:
        seqs = {}
        currentChain = None
